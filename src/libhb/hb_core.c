...
#include <libhb/hbversion.h>
#include <string.h>

#define LIBHB_VERSION_STRING_BASE EMB_STRINGIFY(LIBHB_VERSION_MAJOR) "." EMB_STRINGIFY(LIBHB_VERSION_MINOR) "." EMB_STRINGIFY(LIBHB_VERSION_PATCH)

#if LIBHB_VERSION_IS_RELEASE
#define LIBHB_VERSION_STRING LIBHB_VERSION_STRING_BASE
#else
#define LIBHB_VERSION_STRING LIBHB_VERSION_STRING_BASE "-" LIBHB_VERSION_SUFFIX
#endif

uint32_t libhb_version(void) {
    return LIBHB_VERSION_HEX;
}

const char* libhb_version_string(void) {
    return LIBHB_VERSION_STRING;
}
...
uint8_t hb_get_available_zone_properties_by_type(hb_machine_type_t type,
    ...
    switch (type) {
        ...
        case HMT_WIFI2:
        case HMT_BELL: {
            const hb_zone_property_t EMB_CODE_MODIFIER hzps[] = HB_GPRS_MACHINE_SUPPORTED_ZONE_PROPERTIES;
            memcpy(props, hzps, sizeof(hzps));
            return sizeof(hzps) / sizeof(hb_zone_property_t);
        }
        case HMT_NETMOD: {
            const hb_zone_property_t EMB_CODE_MODIFIER hzps[] = HB_NETMOD_MACHINE_SUPPORTED_ZONE_PROPERTIES;
            memcpy(props, hzps, sizeof(hzps));
            return sizeof(hzps) / sizeof(hb_zone_property_t);
        }
        case HMT_LCD: {
            const hb_zone_property_t EMB_CODE_MODIFIER hzps[] = HB_LCD_MACHINE_SUPPORTED_ZONE_PROPERTIES;
            memcpy(props, hzps, sizeof(hzps));
            return sizeof(hzps) / sizeof(hb_zone_property_t);
        }
        case HMT_WIRED: {
            const hb_zone_property_t EMB_CODE_MODIFIER hzps[] = HB_WIRED_MACHINE_SUPPORTED_ZONE_PROPERTIES;
            memcpy(props, hzps, sizeof(hzps));
            return sizeof(hzps) / sizeof(hb_zone_property_t);
        }
        case HMT_TRUE_COLOR: {
            const hb_zone_property_t EMB_CODE_MODIFIER hzps[] = HB_TRUE_COLOR_MACHINE_SUPPORTED_ZONE_PROPERTIES;
            memcpy(props, hzps, sizeof(hzps));
            return sizeof(hzps) / sizeof(hb_zone_property_t);
        }
        ...
        case HMT_IOT: {
            const hb_zone_property_t EMB_CODE_MODIFIER hzps[] = HB_IOT_MACHINE_SUPPORTED_ZONE_PROPERTIES;
            memcpy(props, hzps, sizeof(hzps));
            return sizeof(hzps) / sizeof(hb_zone_property_t);
        }
        case HMT_GPRS_PHONE: {
            const hb_zone_property_t EMB_CODE_MODIFIER hzps[] = HB_GPRS_PHONE_MACHINE_SUPPORTED_ZONE_PROPERTIES;
            memcpy(props, hzps, sizeof(hzps));
            return sizeof(hzps) / sizeof(hb_zone_property_t);
        }
        case HMT_NB: {
            const hb_zone_property_t EMB_CODE_MODIFIER hzps[] = HB_NB_MACHINE_SUPPORTED_ZONE_PROPERTIES;
            memcpy(props, hzps, sizeof(hzps));
            return sizeof(hzps) / sizeof(hb_zone_property_t);
        }
        default:
...
bool hb_machine_supports_zone_property(hb_machine_type_t type, hb_zone_property_t prop) {
    hb_zone_property_t EMB_XDATA_MODIFIER props[HZP_COUNT];
    uint8_t EMB_DATA_MODIFIER count = hb_get_available_zone_properties_by_type(type, props), i;
    for (i = 0; i < count; i++) {
        if (props[i] == prop) {
            return true;
        }
...
const char* hb_get_zone_format_str(hb_machine_type_t type) {
    uint16_t EMB_DATA_MODIFIER zone = hb_get_max_zone_by_type(type);
    if (10 < zone && zone < 100)
        return "%02d";
    else if (zone < 1000)
        return "%03d";
