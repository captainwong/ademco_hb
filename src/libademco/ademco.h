...
#define ADEMCO_HB_EVENTS_MAP(XX)                               \
    ...
    XX(1827, I_AM_BELL, "主机类型--门铃主机")                  \
    XX(1837, I_AM_TRAIN, "主机类型--铁路预警主机")             \
 ...
 * 1-10: 遥控器
 * 11-30: 中继报警识别码(主机A1,A2,A3,...A20将主机B设为中继目标，则服务器收到任何A*的报警(非异常)，都以控制源(10+*)发送1120紧急报警给B)
 * 31-50: 保留
 * 51-96: 智能家居
 * 97: 定时器自动控制
 * 98: 中转接警中心
...
#define ADEMCO_CONTROL_SOURCES_MAP(XX)                \
    ...
    XX(11, ALARM_RELAY_BASE, "中继报警识别码")        \
    XX(30, ALARM_RELAY_MAX, "中继报警识别码MAX")      \
...
ADEMCO_EXPORT_SYMBOL uint32_t ademco_version(void);

#if ADEMCO_ENABLE_TO_STRING
ADEMCO_EXPORT_SYMBOL const char* ademco_version_string(void);
#endif

ADEMCO_EXPORT_SYMBOL void ademco_print(const ademco_char_t* buf, size_t len);

ADEMCO_EXPORT_SYMBOL uint8_t ademco_encode_signal_strength(uint8_t strength);
ADEMCO_EXPORT_SYMBOL uint8_t ademco_decode_signal_strength(uint8_t bcd_strength);
 ...
 * @note 必须以 '\0'结尾
 * @note 若包含HEX字符，则必须为12字节即 MAC 地址格式，否则不合法
 * @note 若不包含HEX字符，则长度必须在 ADEMCO_PACKET_ACCT_MIN_LEN 和 ADEMCO_PACKET_ACCT_MAX_LEN 之间
 * @note 全0或全F的账号不合法
 */
ADEMCO_EXPORT_SYMBOL bool ademco_is_valid_account(const char* acct);
 ...
 * @return true if valid, false if invalid
 * @note 必须以 '\0'结尾
 * @note 长度必须在 ADEMCO_PACKET_PWD_MIN_LEN 和 ADEMCO_PACKET_PWD_MAX_LEN 之间
 * @note 必须全数字，目前不支持非数字密码
 */
ADEMCO_EXPORT_SYMBOL bool ademco_is_valid_password(const char* pwd);

//////////////////////// ademco_event_t functions ////////////////////////

// 是否主机状态事件
ADEMCO_EXPORT_SYMBOL bool ademco_is_machine_status_event(ademco_event_t ademco_event);

// 是否主机类型事件
ADEMCO_EXPORT_SYMBOL bool ademco_is_machine_type_event(ademco_event_t ademco_event);

// 事件是否需要控制源
ADEMCO_EXPORT_SYMBOL bool ademco_is_event_need_control_source(ademco_event_t ademco_event);

// 获取安定宝事件级别
ADEMCO_EXPORT_SYMBOL ademco_event_level_t ademco_get_event_level(ademco_event_t ademco_event);

// 获取异常恢复事件所对应的异常事件
ADEMCO_EXPORT_SYMBOL ademco_event_t ademco_get_exception_event_by_recover_event(ademco_event_t recover_event);

#if ADEMCO_ENABLE_TO_STRING
ADEMCO_EXPORT_SYMBOL const char* ademco_event_to_string(ademco_event_t ademco_event);
#endif

#if ADEMCO_ENABLE_CHINESE
ADEMCO_EXPORT_SYMBOL const char* ademco_event_to_string_chinese(ademco_event_t ademco_event);
#endif

//////////////////////// ademco_data_t functions ////////////////////////

// make a `DATA` packet and store to `packet`, return length
ADEMCO_EXPORT_SYMBOL size_t ademco_append_data(ademco_char_t* packet,
                                               const char* acct,
                                               ademco_id_t ademco_id,
                                               ademco_event_t ademco_event,
                                               ademco_gg_t gg,
...
ADEMCO_EXPORT_SYMBOL size_t ademco_append_data2(ademco_data_t* ademco_data,
                                                const char* acct,
                                                ademco_id_t ademco_id,
                                                ademco_event_t ademco_event,
                                                ademco_gg_t gg,
...
ADEMCO_EXPORT_SYMBOL size_t ademco_append_data3(ademco_data_t* ademco_data, const char* acct);
...
ADEMCO_EXPORT_SYMBOL ademco_parse_result_t ademco_parse_data(const ademco_char_t* packet,
                                                             size_t packet_len,
                                                             ademco_data_t* ademco_data,
                                                             ademco_parse_error_t* err);

// return 0 for empty packet, CONGWIN_FE100_PACKET_LEN for success
ADEMCO_EXPORT_SYMBOL size_t ademco_data_to_congwin_fe100(ademco_char_t* fe100,
                                                         size_t fe100_len,
...
// init a empty xdat
ADEMCO_EXPORT_SYMBOL void ademco_xdata_init(ademco_xdata_t* xdat);

ADEMCO_EXPORT_SYMBOL bool ademco_xdata_convert(ademco_xdata_t* xdat, ademco_xdata_length_format_t xlf);

#ifndef SWIG
// get valid content address of xdat (except [len])
ADEMCO_EXPORT_SYMBOL const char* ademco_xdata_get_valid_content_addr(const ademco_xdata_t* xdat);
#endif

// get valid content length of xdat (except [len])
ADEMCO_EXPORT_SYMBOL size_t ademco_xdata_get_valid_content_len(const ademco_xdata_t* xdat);

// return 0 if xdat's valid content is exactly the same as [buf, buf+buf_len)
ADEMCO_EXPORT_SYMBOL bool ademco_xdata_memeq(const ademco_xdata_t* xdat,
                                             const void* buf,
                                             size_t buf_len);

// copy xdat content from src to dst, return copied length = src.raw_len
ADEMCO_EXPORT_SYMBOL size_t ademco_xdata_copy(ademco_xdata_t* dst,
                                              const ademco_xdata_t* src);

ADEMCO_EXPORT_SYMBOL bool ademco_make_xdata(ademco_xdata_t* xdat,
...
ADEMCO_EXPORT_SYMBOL bool ademco_is_valid_packet_id(const char* standard,
                                                    const char* id,
                                                    size_t len);

ADEMCO_EXPORT_SYMBOL ademco_packet_id_t ademco_packet_id_from_string(const char* id,
                                                                     size_t len);

ADEMCO_EXPORT_SYMBOL const char* ademco_packet_id_to_string(ademco_packet_id_t id);
...
ADEMCO_EXPORT_SYMBOL size_t ademco_make_empty_data_packet(ademco_char_t* dst_buff,
                                                          size_t len,
                                                          const char* id,
                                                          uint16_t seq,
                                                          const char* acct,
...
ADEMCO_EXPORT_SYMBOL size_t ademco_make_adm_empty_data_packet(ademco_char_t* dst_buff,
                                                              size_t len,
                                                              const char* id,
                                                              uint16_t seq,
                                                              const char* acct,
...
ADEMCO_EXPORT_SYMBOL size_t ademco_make_null_packet(ademco_char_t* buf,
                                                    size_t len,
                                                    uint16_t seq,
                                                    const char* acct,
                                                    ademco_id_t ademco_id);

ADEMCO_EXPORT_SYMBOL size_t ademco_make_ack_packet(ademco_char_t* buf,
                                                   size_t len,
                                                   uint16_t seq,
                                                   const char* acct,
                                                   ademco_id_t ademco_id);

ADEMCO_EXPORT_SYMBOL size_t ademco_make_nak_packet(ademco_char_t* buf,
                                                   size_t len,
                                                   uint16_t seq,
                                                   const char* acct,
                                                   ademco_id_t ademco_id);

ADEMCO_EXPORT_SYMBOL size_t ademco_make_hb_packet(ademco_char_t* buf,
                                                  size_t len,
                                                  uint16_t seq,
                                                  const char* acct,
                                                  ademco_id_t ademco_id,
...
ADEMCO_EXPORT_SYMBOL size_t ademco_make_adm_packet(ademco_char_t* buf,
                                                   size_t len,
                                                   uint16_t seq,
                                                   const char* acct,
                                                   ademco_id_t ademco_id,
...
ADEMCO_EXPORT_SYMBOL size_t ademco_make_null_packet2(ademco_packet_t* pkt,
                                                     uint16_t seq,
                                                     const char* acct,
                                                     ademco_id_t ademco_id);

ADEMCO_EXPORT_SYMBOL size_t ademco_make_ack_packet2(ademco_packet_t* pkt,
                                                    uint16_t seq,
                                                    const char* acct,
                                                    ademco_id_t ademco_id);

ADEMCO_EXPORT_SYMBOL size_t ademco_make_nak_packet2(ademco_packet_t* pkt,
                                                    uint16_t seq,
                                                    const char* acct,
                                                    ademco_id_t ademco_id);

ADEMCO_EXPORT_SYMBOL size_t ademco_make_hb_packet2(ademco_packet_t* pkt,
                                                   uint16_t seq,
                                                   const char* acct,
                                                   ademco_id_t ademco_id,
                                                   ademco_event_t ademco_event,
...
ADEMCO_EXPORT_SYMBOL size_t ademco_make_hb_packet3(ademco_packet_t* pkt);

ADEMCO_EXPORT_SYMBOL size_t ademco_make_adm_packet2(ademco_packet_t* pkt,
                                                    uint16_t seq,
                                                    const char* acct,
                                                    ademco_id_t ademco_id,
                                                    ademco_event_t ademco_event,
...
// err can be NULL; caller should only check err on got ADEMCO_PARSE_RESULT_ERROR
ADEMCO_EXPORT_SYMBOL ademco_parse_result_t ademco_parse_packet(const ademco_char_t* buf,
                                                               size_t len,
                                                               ademco_packet_t* pkt,
                                                               size_t* cb_commited,
                                                               ademco_parse_error_t* err);
 ...
 */
ADEMCO_EXPORT_SYMBOL uint16_t ademco_crc16(const ademco_char_t* buf, size_t len);
#endif
...
/**
 * @brief 将一个十进制数字转换为十六进制字符串
 * @param str 输出字符串，至少 hex_len * 2 个字节空间
 * @param dec 输入的十进制数字
 * @param hex_len 十六进制字符串长度
 * @return 返回转换后的字符串长度
 * @note 类似 `snprintf(str, hex_len * 2 + 1, "%0{hex_len}X", dec)`，但没有 null terminator。
 * @note 函数不会在str末尾补 null terminator。
 * @note str的长度至少为hex_len * 2，否则会崩溃。
 * @note str的内容是大写的十六进制字符。
 * @example 输入 0x12345678, hex_len=3, 输出字符串"678"
 * @example 输入 0x12345678, hex_len=4, 输出字符串"5678"
 * @example 输入 0x5678, hex_len=6, 输出字符串"005678"
 */
ADEMCO_EXPORT_SYMBOL size_t ademco_dec_to_hex_str(char* str, uint32_t dec, uint8_t hex_len);

/**
 * @brief 将一串以高低字节表示的十六进制数组转换为10进制数字符串
 * @param str 输出字符串，至少len的2倍空间
 * @param arr 输入的十六进制数组
 * @param len 数组长度
 * @return 返回转换后的字符串长度
 * @note 每个字节的高四位和低四位若不大于9，将该四位表示的数字以10进制ascii码填入str，否则停止转换。
 * @note 函数不会在str末尾补 null terminator。
 * @note 调用者应确保str有足够空间，至少len的2倍，否则会崩溃。
 * @example 输入 0x24 0x08 0x88 0x10 0x1f 0xff，输出字符串"18240888101"
 */
ADEMCO_EXPORT_SYMBOL size_t ademco_hilo_array_to_dec_str(ademco_char_t* str,
                                                         const uint8_t* arr,
                                                         size_t len);
 ...
 * @param len 数组长度
 * @return 返回转换后的字符串长度
 * @note 每个字节的高四位和低四位若不大于9，将该四位表示的数字以10进制ascii码填入str，否则：
 * 若定长6字节包含a-f，说明是WiFi主机。
 * @note 函数不会在str末尾补 null terminator。
 * @note 调用者应确保str有足够空间，至少len的2倍，否则会崩溃。
 * @example 输入数组：0x18 0x24 0x08 0x88 0x10 0x1f 0xff，输出字符串"18240888101"
 * @example 输入数组：0x24 0x94 0x94 0x4c 0x48 0x15 0xff，输出字符串"2494944C4815"
 */
ADEMCO_EXPORT_SYMBOL size_t ademco_hilo_array_to_hex_str(ademco_char_t* str,
                                                         const uint8_t* arr,
                                                         size_t len);

/**
 * @brief 比较一个高低字节表示的数组和一个字符串是否相等
 * @param arr 输入的高低字节数组
 * @param len 数组长度
 * @param str 输入的字符串
 * @return true if equal, false if not
 * @note 过程类似先调用 `ademco_hilo_array_to_hex_str` 将数组转换为字符串，
 *       然后比较转换后的字符串和输入的字符串是否相等。
 */
ADEMCO_EXPORT_SYMBOL
bool ademco_hilo_array_streq(const uint8_t* arr, size_t len,
                             const char* str);

/**
 * @brief 将一个10进制数字符串转为高低字节表示的数组，节省空间
 * @param arr 输出数组，至少len字节空间
 * @param len 数组长度
 * @param str 输入的10进制数字符串
 * @return 返回转换后的数组长度
 * @note str应只包含'0'-'9'，否则失败返回0
 * @note str长度若大于len * 2，str[len]及之后的内容将被舍弃以避免溢出
 * @note 转换后的长度若不满len，则以0xF补满。
 * @example 输入字符串 "123ABC"，返回0
 * @example 输入字符串 "18240888101", len=9, 则arr内容为 0x18 0x24 0x08 0x88 0x10 0x1f 0xff 0xff 0xff, return 9
 * @example 输入字符串 "12345678901234567890", len=9, 则arr内容为 0x12 0x34 0x56 0x78 0x90 0x12 0x34 0x56 0x78, return 9
 * @example 输入NULL，返回 9个 0xFF
 */
ADEMCO_EXPORT_SYMBOL size_t ademco_dec_str_to_hilo_array(uint8_t* arr,
                                                         size_t len,
                                                         const char* str);
 ...
 * @example 输入NULL，返回 9个 0xFF
 */
ADEMCO_EXPORT_SYMBOL size_t ademco_hex_str_to_hilo_array(uint8_t* arr,
                                                         size_t len,
                                                         const char* str);

/**
 * @brief 将一串字节流转换为可打印形式
 * @param str 输出字符串，至少len * 2个字节空间
 * @param arr 输入的字节流数组
 * @param len 数组长度
 * @return 返回转换后的字符串长度
 * @note 函数不会在str末尾补 null terminator
 * @note 调用者应确保str有足够的容量，至少len * 2，否则会崩溃
 * @example 输入 arr=[0xEB, 0xBA, 0x3F], len=3, 则str = "EBBA3F"，返回6
 * @example 输入 arr="123", len=3, 则str = "313233"，返回6
 */
ADEMCO_EXPORT_SYMBOL size_t ademco_hex_array_to_str(char* str,
                                                    const uint8_t* arr,
                                                    size_t len);

/**
 * @brief 将一串字符串（内容为'0'-'9', 'A'-'F', 'a'-'f'）转为字节流
 * @param arr 输出字节流数组，至少strlen(str) * 2个字节空间
 * @param str 输入的十六进制字符串
 * @param padding 若输入字符串长度为奇数，使用padding补足字节流
 * @param padding 应小于等于0x0F
 * @return 返回转换后的字节流长度
 * @note 若输入字符串包含非十六进制字符，返回0
 * @note 调用者应确保arr有足够的容量，至少strlen(str) * 2，否则会崩溃
 * @example 输入字符串str="EBBA3F"，输出arr = [0xEB, 0xBA, 0x3F]，返回 3
 * @example 输入字符串str="ABC"，padding = 0，输出arr=[0xAB, 0xC0]，返回2
 * @example 输入字符串str="ABC"，padding = 0x0F，输出arr=[0xAB, 0xCF]，返回2
 */
ADEMCO_EXPORT_SYMBOL size_t ademco_hex_str_to_array(uint8_t* arr,
                                                    const char* str,
                                                    uint8_t padding);

/**
 * @brief 将一串字符串（内容为'0'-'9', 'A'-'F', 'a'-'f'）转为字节流
 * @param arr 输出字节流数组，至少len个字节空间
 * @param str 输入的十六进制字符串
 * @param len 输出字节流的长度
 * @param padding 若输入字符串长度为奇数，使用padding补足字节流
 * @param padding 应小于等于0x0F
 * @return 返回转换后的字节流长度
 * @note 功能同 ademco_hex_str_to_array, 但允许输入字符串长度超过len * 2, 若 strlen(str) > len * 2, str[len]及之后的内容将被舍弃以避免溢出
 * @example 输入字符串str="ABCDE", len = 4, padding = 0x0F, 输出arr=[0xAB, 0xCD], return 2
 */
ADEMCO_EXPORT_SYMBOL size_t ademco_hex_str_to_array_n(uint8_t* arr,
                                                      const char* str,
                                                      size_t len, uint8_t padding);

/**
 * @deprecated 网线主机已升级固件，不建议再使用此函数
 * @brief 将一串字符串（内容为'0'-'9', 'A'-'F', 'a'-'f'）转为字节流
 * @param arr 输出字节流数组，至少len个字节空间
 * @param str 输入的十六进制字符串
 * @param len 输出字节流的长度
 ...
 * @param padding 应小于等于0x0F
 * @return 返回转换后的字节流长度
 * @note 功能同 ademco_hex_str_to_array_n, 但允许输入字符串包含非十六进制字符，
 * 即'0'-'9','a'-'f','A'-'F'之外的内容，以 padding 替换
 * 例如：str="AB\xFFD", len=4, padding=0x0F, 则 arr=[ 0xAB, 0xFD ], return 2
 * 网线主机 0d 00 命令，接警中心账号部分，有可能是这种
 */
ADEMCO_EXPORT_SYMBOL size_t ademco_hex_str_to_array_n_allow_non_hex_str(uint8_t* arr,
                                                                        const char* str,
                                                                        size_t len,
                                                                        uint8_t padding);
